...
ggsurv <- function(s, CI = 'def', plot.cens = T, surv.col = 'gg.def',
  ...
  ggsurv.s <- function(s, CI = 'def', plot.cens = T, surv.col = 'gg.def',
    ...
    pl <- if(plot.cens == T & length(dat.cens) > 0){
      pl + ggplot2::geom_point(data = dat.cens, aes(y = surv), shape = cens.shape,
                      col = cens.col)
  ...
  ggsurv.m <- function(s, CI = 'def', plot.cens = T, surv.col = 'gg.def',
    ...
    pl <- ggplot2::ggplot(dat, aes(x = time, y = surv, group = group)) +
      ...
      ggplot2::geom_step(aes(col = group, lty = group))
    ...
    pl <- if(CI == T) {
      ...
      } else{pl +  ggplot2::geom_step(aes(y = up, lty = group), col = surv.col) +
